...
# Simulacion de carga progresiva
for i in range(100):
    time.sleep(0.01)
    progress_bar.progress(i + 1)
    status_text.text(f"Cargando dataset... {i+1}%")

# Carga real (SIN modificar tu logica)
    ...
    with col1:
        ...
        if st.button("Ver vecinos mas cercanos (Manhattan)"):
            ...
            if vecinos:
                ...
                ## MOSTRAR CON FORMATO DECIMAL de 10 decimales
                df_formateado = df_vecinos.style.format({"Distancia Manhattan": "{:.10f}"})
                st.dataframe(df_formateado, use_container_width=True)

                ## MOSTRAR CON FORMATO DECIMAL 2 decimales
                ## ELiminar df_formateado = df-vecinos-style-format({"Distancia Manhattan": "{:.10f}"})
                ## USAR st.dataframe(df_vecinos, use_container_width=True)
            else:
